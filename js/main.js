...
import { registerMessage } from "./register.js";
import { createVisitCard } from "./ajax/api.js";
import { getCards, getCard } from "./ajax/api.js";
...
mainText.textContent = "You are not registered";
if (!sessionStorage.getItem("token")) {
  headerText.innerText = "Click registration button to start";
} else {
  ...
  enterBtn.addEventListener("click", async (event) => {
    ...
    if (!regToken || regToken === "Incorrect username or password") {
      ...
      modal.show(
        ...
        registerMessage("Registration completed successfully.", "success")
      );
...
// btnResetForm.append(this.elem)
// btnResetForm.setAttribute("data-tooltip", "Reset");
...
//   `.user-card[data-visit-type="Cardiolog"]`
// );
